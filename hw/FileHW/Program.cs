...
namespace Homework1
{
    class Program
    {
        ...
        {
            string path = @"C:\Users\Адиль\source\repos\FileHW\FileHW\";

            string[] words;
            List<int> numbers = new List<int>();

            using (StreamReader stream = new StreamReader(path + "file1.txt"))
            {
            ...
            if (IsFib(numbers))
            {
                ...
                using (FileStream stream = new FileStream(path + "file2.txt", FileMode.Create))
                {
